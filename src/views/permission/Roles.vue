<template>
    <div>
        ...
        <el-dialog title="分配权限" :visible.sync="setRightDialogVisable" width="50%" @close="setRightDialogClosed">
            <el-tree :data="rightsTree" :props="treeProps" ref="treeRef" show-checkbox node-key="id"
                     :default-checked-keys="defKeys" default-expand-all></el-tree>
            <span slot="footer" class="dialog-footer">
                <el-button @click="setRightDialogVisable = false">取消</el-button>
                <el-button @click="setRoleRights">确定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            async setRoleRights() {
                ...
                param.append('rightIds', idStr)
                const {data: res} = await this.$http.post('/role/setRights', param)
                if (res.code !== 200) {
                    return this.$message.error(res.msg)
                }
